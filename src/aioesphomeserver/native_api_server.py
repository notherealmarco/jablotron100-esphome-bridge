...
class NativeApiConnection:
    ...
    async def handle_next_message(self):
        try:
            ...
            if msg is None:
                # Unknown message type, or the peer hit EOF (read_next_message
                # returns None). Returning here would send the caller's
                # `while self.running` loop straight back into read_next_message,
                # which at EOF returns synchronously with no yielding await point,
                # starving the event loop for every other client.
    ...
    async def handle_hello(self, msg):
        resp = HelloResponse(
            api_version_major=1,
            api_version_minor=self.server.api_version_minor,
        )
        await self.write_message(resp)

    async def handle_authentication(self, msg):
        # Modern ESPHome (2026.1+) has no password authentication: the client
        # is authenticated immediately after hello. Answer success regardless.
...
class NativeApiServer(BasicEntity):
    def __init__(self, *args, port=6053, api_version_minor=18, **kwargs):
