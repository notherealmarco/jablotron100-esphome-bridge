...
def _multicast_interface_addresses() -> list[str]:
    """IPv4 addresses of up, multicast-capable, non-tunnel interfaces.

    Skips POINTOPOINT links (WireGuard/VPN tunnels): announcing mDNS over them
    fails in the kernel and only spams the log.
    """
    addresses = []
    for _ifindex, name in socket.if_nameindex():
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            ifreq = struct.pack("256s", name[:15].encode("utf-8"))
            flags = struct.unpack("H", fcntl.ioctl(sock.fileno(), _SIOCGIFFLAGS, ifreq)[16:18])[0]
            if (flags & (_IFF_UP | _IFF_MULTICAST)) == (_IFF_UP | _IFF_MULTICAST) and not (flags & _IFF_POINTOPOINT):
                address = socket.inet_ntoa(fcntl.ioctl(sock.fileno(), _SIOCGIFADDR, ifreq)[20:24])
...
class Device:
    ...
    async def build_device_info_response(self):
        return DeviceInfoResponse(
            ...
            esphome_version=self.project_version or "",
            model=self.model,
            manufacturer=self.manufacturer,
            friendly_name=self.friendly_name or self.name,
            webserver_port=self.webserver_port or 0,
            devices=self.sub_devices,
        )
    ...
    async def run(self, api_port, web_port=None):
        from . import NativeApiServer
    ...
    async def register_zeroconf(self, port):
        try:
            interfaces = _multicast_interface_addresses()
            zeroconf = AsyncZeroconf(interfaces=interfaces) if interfaces else AsyncZeroconf()
