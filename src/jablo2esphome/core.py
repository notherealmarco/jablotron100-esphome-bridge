...
from typing import Any, BinaryIO, Callable, Dict, Final, List, Mapping
from .errors import (
	InvalidBatteryLevel,
...
StateType = bool | float | int | str | None


def callback(func: Callable) -> Callable:
	"""Identity wrapper; Home Assistant's real one only tags for tracing."""
	return func
...
class Jablotron:
	...
	def __init__(self, runtime, config_entry_id: str, config: Mapping[str, Any], options: Mapping[str, Any]) -> None:
		self._rt = runtime
		...
		self._store = async_get_store(self._rt, STORAGE_VERSION)
	...
	async def initialize(self) -> None:
		...
		self._rt.bus.async_listen(EVENT_SHUTDOWN, shutdown_event)
		...
		if self._config[CONF_SERIAL_PORT] == AUTODETECT_SERIAL_PORT:
			...
			if not await self._rt.async_add_executor_job(os.path.exists, self._serial_port):
				LOGGER.warning(
	...
	def central_unit_device_id(self) -> str | None:
		return self._rt.device_id_for_identifier((DOMAIN, self.central_unit().unique_id), self._config_entry_id)
	...
	def modify_alarm_control_panel_section_state(self, section: int, state: AlarmControlPanelState, entered_code: str | None) -> None:
		...
		def after_modify_callback(_: datetime.datetime) -> None:
			...
			self._rt.async_add_executor_job(
				self._send_packet,
		...
		def after_login_callback(_: datetime.datetime | None) -> None:
			...
			if packets_to_send:
				self._rt.async_add_executor_job(self._send_packets, packets_to_send)

			self._rt.async_call_later(1.0, after_modify_callback)
		...
		if code != configured_code:
			...
			self._rt.call_soon_threadsafe(
				self._rt.async_call_later,
				1.0,
			...
			self._rt.call_soon_threadsafe(after_login_callback, None)
	...
	def _send_signal_entities_added(self) -> None:
		if self._main_thread == threading.current_thread():
			self._rt.async_dispatcher_send(self.signal_entities_added())
		else:
			self._rt.dispatcher_send(self.signal_entities_added())
	...
	async def _create_devices(self) -> None:
		for device_number in range(1, self._config[CONF_NUMBER_OF_DEVICES] + 1):
			...
			if self._is_device_ignored(device_number):
				self._rt.async_remove_ignored_device((DOMAIN, device_id), self._config_entry_id)
	...
	def _login_error(self) -> None:
		self._rt.bus.fire(EVENT_WRONG_CODE)
	...
	def _send_packet_by_stream(self, packet: bytes) -> None:
		if self._main_thread == threading.current_thread():
			# On the event loop thread. A blocked serial write here would stall
			# every connected client (and the close_stream rescue callback could
			# never fire because the loop itself is stuck), so the blocking I/O
			# must run on the executor instead.
			self._rt.async_add_executor_job(self._write_and_close_stream, packet)
		else:
	...
	def _write_and_close_stream(self, packet: bytes) -> None:
		stream = self._open_write_stream()
	...
	def _update_entity_state(self, entity_id: str, state: StateType | AlarmControlPanelState, store_state: bool = True) -> None:
		if entity_id in self.hass_entities:
			self._rt.call_soon_threadsafe(self._apply_entity_state, entity_id, state, store_state)
		else:
	...
	def _store_central_unit_data(self) -> None:
		...
		with self._state_lock:
			...
			self._stored_data[unique_id][STORAGE_CENTRAL_UNIT_KEY] = copy.deepcopy(self._central_unit_data)
		self._store_data_to_store_threadsafe()
	...
	def _store_devices_data(self) -> None:
		...
		with self._state_lock:
			...
			self._stored_data[unique_id][STORAGE_DEVICES_KEY] = copy.deepcopy(self._devices_data)
		self._store_data_to_store_threadsafe()
	...
	def _data_to_store(self) -> dict:
		with self._state_lock:
			return copy.deepcopy(self._stored_data)
	...
	def _parse_device_signal_strength_from_device_info_subpacket(packet: bytes) -> int:
		# Same 5-bit signal + flag encoding as the device status packet: bits
		# 5-7 carry flags (they change between readings), so the signal must be
		# masked off before scaling or values above 100% leak out.
		return (Jablotron.bytes_to_int(packet[2:3]) & 0x1f) * SIGNAL_STRENGTH_STEP
	...
	async def _detect_serial_port(self) -> str | None:
		try:
			possible_paths = await self._rt.async_add_executor_job(os.listdir, HIDRAW_PATH)
		except OSError as ex:
			LOGGER.debug("Failed to list %s: %s", HIDRAW_PATH, ex)
			return None
