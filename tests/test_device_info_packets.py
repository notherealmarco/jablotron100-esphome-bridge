...
@pytest.mark.parametrize(
	("signal_byte", "expected_signal_strength"),
	[
		pytest.param(0x12, 90, id="flags-clear"),
		# Same low 5 bits (18) with high flag bits set: the indoor siren reads
		# 0xB2 on some packets and 0x12 on others, which surfaced as 890%.
		pytest.param(0xB2, 90, id="flag-bits-set"),
		pytest.param(0x0A, 50, id="mid"),
		pytest.param(0x00, 0, id="zero"),
	],
)
def test_parse_device_signal_strength_from_info_subpacket_masks_flag_bits(
	signal_byte: int,
	expected_signal_strength: int,
) -> None:
	subpacket = bytes([0x01, 0x02, signal_byte, 0x00])

	assert (
		Jablotron._parse_device_signal_strength_from_device_info_subpacket(subpacket)
